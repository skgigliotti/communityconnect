...
export const getDistance = (targetLocation: Coordinates, myLocation: Coordinates): number | undefined => {
    ...
    const metersToMiles = (meters: number) => meters / NUM_METERS_IN_ONE_MILE;
...
export const getCloserResource = (a: Coordinates, b: Coordinates, currentPosition: Coordinates) => {
    const distanceA = getDistance(a, currentPosition);
    const distanceB = getDistance(b, currentPosition);
...
export const getCloserName = (a: { name: string }, b: { name: string }): number => {
